...
| Backup and disaster recovery | STALE | Local rehearsed 2026-08-19. **Production Postgres now proven 2026-08-21:** `scripts/backup_railway.sh` took a real Railway dump and rebuilt it into a throwaway Postgres 18, row counts matching (72 users, 3406 attendance, 238 grades, 1 payment) — and Railway's own daily volume backups are enabled. Two things keep this STALE, not VERIFIED: (1) the **MinIO** object store (the register's known data-loss failure mode) still has no off-box copy; (2) the run revealed production is on a **pre-multi-tenancy schema** (no `schools` table), i.e. this branch is undeployed — so what restores today is the old app, not the hardened one. |
