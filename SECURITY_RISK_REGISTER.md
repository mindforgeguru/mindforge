...
| Multi-factor authentication | STALE | TOTP (RFC 6238, stdlib, checked against all six official vectors) for **admin and owner**, opt-in, now with an enrolment screen reachable from the admin profile and the owner console — QR plus a typed key, single-use recovery codes, and screen-capture blocked while the secret is on screen. Two-step enrolment so an abandoned setup cannot lock the account; disable requires MPIN *and* a factor. Verified end to end in the running app. **STALE for one specific reason: no real authenticator app has scanned the QR.** Every code in testing was generated by the same implementation that checks it, which is marking your own homework. Other roles keep a single factor by design. |
