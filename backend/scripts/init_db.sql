...
-- Partial unique index: username unique only among non-deleted users
-- (allows re-registration after soft-delete during academic year rollover)
...
-- Idempotent migration: add answer_key_url column if it doesn't exist
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'tests' AND column_name = 'answer_key_url'
    ) THEN
        ALTER TABLE tests ADD COLUMN answer_key_url VARCHAR(500);
    END IF;
END $$;
...
CREATE OR REPLACE VIEW active_students AS
    SELECT u.*, sp.grade, sp.profile_pic_url, sp.parent_user_id
    FROM users u
...
VALUES (
    'admin',
    '$2b$12$EixZaYVK1fsbw1ZfbX3OXePaWxn96p36WQoeG6Lruj3vjPGga31lW', -- bcrypt of '123456'
    'admin',
    TRUE,
    TRUE
